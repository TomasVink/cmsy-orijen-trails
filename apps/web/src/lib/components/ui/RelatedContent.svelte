<script lang="ts">
  import type { Trail, Post, Influencer } from '$lib/payload'
  ...
  const trails = $derived(
    (related?.trails ?? []).filter((t): t is Trail => typeof t === 'object'),
  )
  const posts = $derived(
    (related?.posts ?? []).filter((p): p is Post => typeof p === 'object'),
  )

  const hasContent = $derived(influencers.length > 0 || trails.length > 0 || posts.length > 0)
</script>
