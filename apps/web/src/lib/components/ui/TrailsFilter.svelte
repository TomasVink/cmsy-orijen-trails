...
<div class="border-b border-orijen-gray/20">
  <!-- Toggle bar -->
  <div class="max-w-6xl mx-auto px-4">
    <button
      onclick={() => (open = !open)}
      class="flex items-center gap-2 py-3 text-xs font-bold uppercase tracking-widest text-orijen-gray hover:text-orijen-red transition-colors cursor-pointer"
    >
      <span>{f?.difficulty ? 'Filters' : 'Filters'}</span>
      {#if activeCount > 0}
      ...
      <div class="max-w-6xl mx-auto px-4 pb-4">
        <div class="flex flex-wrap gap-6 items-end">
          ...
          {#if activeCount > 0}
            <button
              onclick={clearAll}
              class="px-3 py-1.5 text-xs font-bold uppercase tracking-widest text-orijen-red hover:underline self-end mb-0.5 cursor-pointer"
            >
