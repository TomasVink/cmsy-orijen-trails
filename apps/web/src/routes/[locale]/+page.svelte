...
<svelte:head>
  <title>{preview.data?.seo?.title ?? preview.data?.title ?? 'ORIJEN Trails'}</title>
...
{#if preview.isLoading}
  <div class="fixed inset-0 flex items-center justify-center bg-white/80 z-50">
    <span class="font-sans text-sm text-orijen-gray uppercase tracking-widest animate-pulse">
...
{#if preview.data?.layout?.length}
  <BlockRenderer
    blocks={preview.data.layout}
    form={data.form}
    signUpForm={data.signUpForm}
    patchRequestForm={data.patchRequestForm}
    events={data.events}
    trailsWithStores={data.trailsWithStores}
  />
{:else if !data.page}
  <div class="min-h-screen flex items-center justify-center">
    <p class="font-sans text-orijen-gray">
      No home page found — create a page with slug <code class="font-mono text-sm bg-black/5 px-1"
        >home</code
      > in the CMS.
    </p>
  </div>
